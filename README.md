# THỰC HÀNH 05: QUẢN LÝ PHIÊN BẢNG VỚI GIT/GITHUB

---

### 1. Danh Sách File Nội Dung Thực Hành
Dưới đây là các file được tạo ra từ Nhiệm vụ 1.1 đến 3.1:

* **BaoCao_LyThuyet.pdf:** File PDF chứa nội dung thực hành 1.1 và 1.2.
* **infographic.png:** Ảnh infographic tóm tắt (Nhiệm vụ 2.1).
  **giấy phép CC BY** :https://creativecommons.org/licenses/by/4.0/
* **index.html:** Trang web hiển thị nội dung số (Nhiệm vụ 3.1).
* **[Video MP4/Google Drive]:** Video tạo bởi CapCut, tài liệu từ Pexels/FreeSound (CC0).
  **Giấy phép CC BY** :https://creativecommons.org/licenses/by/4.0/

---

### 2. Trích Dẫn Nguồn AI & Bản Quyền

* **Công cụ AI được sử dụng:**
    * **Canva AI:** Hỗ trợ thiết kế Infographic.
    * **GitHub Copilot:** Hỗ trợ tạo mã nguồn HTML.
* **Giấy phép (License):** Nội dung này được cấp phép theo **Creative Commons Attribution 4.0 International (CC BY 4.0)**.
    https://creativecommons.org/licenses/by/4.0/
